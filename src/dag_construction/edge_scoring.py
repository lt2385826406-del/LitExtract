...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'KG_and_Causal'))
from .cooccurrence_miner import run_step1
from .dag_causal_builder import CausalHypothesisGraph
...
print(f"  Process types: {len(total_process_counter)} ({sum(total_process_counter.values())} times)")
print(f"  Phase types: {len(total_phase_counter)} ({sum(total_phase_counter.values())} times)")
print(f"  Property types: {len(total_prop_counter)} ({sum(total_prop_counter.values())} times)")
...
#
print(f"\n{'='*70}")
print("Step 1: Co-occurrence statistics (min_freq=2)...")
print(f"{'='*70}")
...
print(f"{'='*70}")
print(f"  Step1 candidate edges:   {result['step1_added']}")
...
print(f"\nStatistics info saved to: {stats_path}")
